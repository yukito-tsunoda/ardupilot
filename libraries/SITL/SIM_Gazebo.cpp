...
{
    static int nbConsecutiveSuccess = 0;
    servo_packet pkt;
    int i;
    ...
    } else {    // sent == sizeof(pkt), so it is good
        if ((_link_status == ERR_FATAL) ||
            (_link_status == ERR_BYTES_LOST)) {
            nbConsecutiveSuccess++;

            // Connection is assumed restored if 3 consecutive messages are well handled
            if (nbConsecutiveSuccess > 3) {
                fprintf(stdout, "Connection restored on control socket\n");
                _link_status = RUNNING;                     // This message sending was fine, resets the status
                nbConsecutiveSuccess = 0;
            }
        }
    }
}
